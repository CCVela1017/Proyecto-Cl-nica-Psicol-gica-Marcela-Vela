...
import compras
global texto_imagen
from compras_files import agregar_existentes


def open_producto_existen():
    agregar_existentes.main()


def open_new_product():
    compras.main()
...
def main():
    ...
    button_ingreso = customtkinter.CTkButton(master=frame, text="Producto Existente", fg_color=color, height=150,
                                             command=open_producto_existen)
    button_ingreso.pack(pady=100, padx=10)
    button_ingreso.place(x=10, y=100)

    button_mostrar = customtkinter.CTkButton(master=frame, text="Producto Nuevo", fg_color=color, height=150,
                                             command=open_new_product)
